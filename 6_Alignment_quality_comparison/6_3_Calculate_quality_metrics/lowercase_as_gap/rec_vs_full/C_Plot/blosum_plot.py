# Plot BLOSUM62 score comparison distributions (percentage improvement and absolute difference) for REC vs FULL alignments (lowercase as gaps)
...
# Input file containing REC vs FULL BLOSUM comparison
input_path = "../B_Merge_and_comparison/results_merge/blosum_rec_vs_full_comparison.csv"
...
df_percent_long = pd.DataFrame({
    ...
    "value": df["REC_vs_FULL_blosum_%_improvement"]
})
...
plt.title("Distribution of percentage change in BLOSUM62 score (REC vs FULL) (lowercase as gap)")
...
df_diff_long = pd.DataFrame({
    "comparison": ["REC vs FULL"] * len(df),
    "value": df["REC_vs_FULL_blosum_diff"]
})
...
plt.ylabel("Number of alignments")
plt.title("Distribution of BLOSUM62 score differences (REC vs FULL) (lowercase as gap)")
plt.xticks(rotation=45)
plt.tight_layout()
