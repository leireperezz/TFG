# Remove all-gap columns from filtered alignments, treating lowercase letters as gaps
...
for dataset_name, (input_dir, output_dir) in datasets.items():
    ...
    for filename in os.listdir(input_dir):
        ...
        # Convert "." and lowercase letters to "-"
        df_pos = df_pos.apply(
            lambda col: col.map(
                lambda x: "-" if (str(x) == "." or str(x).islower()) else str(x)
            )
        )
