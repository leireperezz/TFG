...
def pos_align(gnomad_df):
    ...
    print(f"\n -The seleted alignment, was not found in {cases:,} cases.\n\n")
    return gnomad_df
...
for gnomad_file in gnomad_file_list:
    ...
    #gnomad.to_csv(os.path.join(output_path, f"gnomad{version_pattern}_with_pfams_no_align.txt"), sep="\t",index=False)
